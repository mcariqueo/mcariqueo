# Marcial Cariqueo Arriagada

**Clinical Pharmacist | PhD Candidate in Translational Medicine | PK/PD & Pharmacometrics | Critical Care Research**

[View portfolio page](./index.html) | [Download CV](./Marcial_Cariqueo_CV_2026.pdf) | [LinkedIn](https://www.linkedin.com/in/marcialcariqueo)

I work at the intersection of clinical pharmacology, critical care, and data-driven translational research, with a focus on optimizing anti-infective therapy in severe respiratory infections.

My current work combines population pharmacokinetics, PK/PD modeling, therapeutic drug monitoring, real-world clinical data, and multicenter clinical research to support evidence-based decisions in complex patients.

## Current Focus

- PhD research in Translational Medicine at Universitat de Barcelona and IDIBAPS - Hospital Clinic de Barcelona.
- Population PK/PD modeling of anti-infectives in severe respiratory infections.
- Real-world evidence from pneumonia and respiratory infection cohorts.
- Clinical study coordination, patient screening, protocol implementation, and REDCap-based data workflows.
- Critical care pharmacotherapy, therapeutic drug monitoring, and model-informed dosing.

## Core Expertise

- **Pharmacometrics:** Population PK, PK/PD, model-informed drug development, therapeutic drug monitoring.
- **Clinical Research:** ICH-GCP, multicenter studies, study operations, clinical data quality control.
- **Healthcare Data Science:** Clinical data analysis, reproducible workflows, predictive modeling.
- **Therapeutic Areas:** Severe respiratory infections, pneumonia, anti-infectives, intensive care, vaccine effectiveness.

## Technical Toolkit

![R](https://img.shields.io/badge/R-276DC3?style=flat&logo=r&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-336791?style=flat&logo=postgresql&logoColor=white)
![REDCap](https://img.shields.io/badge/REDCap-8A1538?style=flat)
![NONMEM](https://img.shields.io/badge/NONMEM-4B5563?style=flat)
![PsN](https://img.shields.io/badge/PsN-4B5563?style=flat)
![Clinical Trials](https://img.shields.io/badge/Clinical%20Trials-0F766E?style=flat)
![PK%2FPD](https://img.shields.io/badge/PK%2FPD-7C3AED?style=flat)

## Selected Research Areas

- Population pharmacokinetics and pharmacodynamics of anti-infectives.
- Antimicrobial optimization in critically ill patients.
- Real-world evidence in pneumonia and respiratory infections.
- Clinical trial coordination and clinical data management.
- Translational research from preclinical PK/PD models to patient-level evidence.

## Publications & Recognition

- Publications in antimicrobial therapy, critical care, sepsis, pharmacotherapy complexity, and intensive care practices.
- Automatically generated publication list: [`source-materials/publications/publications.md`](./source-materials/publications/publications.md)
- Research awards for best poster and oral presentation.
- Experience across hospital pharmacy, ICU pharmacotherapy, clinical research, and translational medicine.

## Contact

- LinkedIn: [marcialcariqueo](https://www.linkedin.com/in/marcialcariqueo)
- Email: [marcial.cariqueo@gmail.com](mailto:marcial.cariqueo@gmail.com)
- CV: [Marcial_Cariqueo_CV_2026.pdf](./Marcial_Cariqueo_CV_2026.pdf)

## Portfolio Source Materials

- Public/shareable inputs for improving this profile: [`source-materials/`](./source-materials/)
- Private local-only inputs ignored by Git: `private-materials/`
- Living profile brief and improvement ideas: [`PROFILE_BRIEF.md`](./PROFILE_BRIEF.md)
- Profile change history: [`PROFILE_CHANGELOG.md`](./PROFILE_CHANGELOG.md)
